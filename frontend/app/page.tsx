...
export default async function Page(){
  let data:any[]=[];let weekly:any[]=[];let error="";
  try{
    [data,weekly]=await Promise.all([getJSON<any[]>("/analytics/fitness"),getJSON<any[]>("/analytics/weekly?weeks=12")]);
  }catch(e:any){
    // Previously swallowed: an unreachable API rendered a dashboard of zeros that looked
    // like a real "no training yet" state.
  ...
  return <><h1>Training dashboard</h1><p className="muted">Current state plus sport-aware metabolic and terrain load.</p>
    {error&&<div className="card section errorText"><strong>Dashboard data unavailable.</strong> {error}<div className="muted">Check that the API is running and that NEXT_PUBLIC_API_URL / API_URL_INTERNAL point at it.</div></div>}
